...
import { TRANSFER, TRANSFERDATA, TYNativeBridge } from './bridgeUtils';
...
const TuyaSingleTransferApi = {
  ...
  registerTransferDataListener(onSuccess, onError) {
    ...
    return TYNativeBridge.on(TYNativeBridge.bridge(TRANSFERDATA, ''), data => {
  ...
  registerTransferCallback(onConnectSuccess, onConnectError) {
    ...
    return TYNativeBridge.on(TYNativeBridge.bridge(TRANSFER, ''), data => {
      if (data.type === 'onSuccess') {
