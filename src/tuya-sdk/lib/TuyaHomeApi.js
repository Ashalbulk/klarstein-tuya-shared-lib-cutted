...
import { HOMEDEVICESTATUS, HOMESTATUS, TYNativeBridge, WARNMESSAGEARRIVED } from './bridgeUtils';
...
const TuyaHomeApi = {
  ...
  registerHomeDeviceStatusListener(
    ...
    return TYNativeBridge.on(TYNativeBridge.bridge(HOMEDEVICESTATUS, params.homeId), data => {
  ...
  unRegisterHomeStatusListener(params, listener) {
    HomeNativeApi.unRegisterHomeStatusListener(params);
    TYNativeBridge.off(listener);
  },
  ...
  registerProductWarnListener(params, onWarnMessageArrived) {
    HomeNativeApi.registerProductWarnListener(params);
    return TYNativeBridge.on(TYNativeBridge.bridge(WARNMESSAGEARRIVED, params.homeId), data => {
